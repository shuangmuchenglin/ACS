...
double Mutual_Distance1[N][N];	//表示两两城市之间的距离
double Cm1;		//信息素局部更新时候使用的的常量，由贪心算法得到

void Read_opt_path1(char a[])  //读取公布的最优路径
{
	a = strcat(a, ".opt.tour");
	FILE* fp;
...
void Read_Coordinate1(char a[])   //读取城市坐标
{
	a = strcat(a, ".tsp");
	FILE* fp;
	char str[100];  //暂存读取的字符串
...
void Print_bestpath1()  //打印已公布的最优解路径及长度
{
	int i;
	cout << "已公布最优路径为：" << endl;
	for (i = 0; i < N - 1; i++)
	...
	cout << "最优解最小路径长度为：7542" << endl<<endl<<endl;
}

int calculateDistance1(int i, int j)//计算两个城市之间的距离(欧氏距离)
{
...
void CalculateDistance1()   //用矩阵表示两两城市之间的距离
{
	for (int i = 0; i < N; i++)
	{
		for (int j = 0; j < N; j++)
...
double calculateSumOfDistance1(int* tour) //获得经过n个城市的路径长度  
{
	double sum = 0;
	for (int i = 0; i< N; i++)
	{
...
double CalAdjacentDistance1(int node)//辅助函数，给一个节点由最近邻距离方法计算长度（计算Cm1）
{
	double sum = 0.0;
	int visitedNode[N];
	for (int j = 0; j < N; j++)
	...
	do
	{
		...
		if (nextNode >= 0)
		{
			sum += Mutual_Distance1[currentNode][nextNode];  //Mutual_Distance1为两城市间的距离矩阵
...
public:
	ACS1()
	{}  //构造函数
	void InitParameter1(double value);	//初始化各路径信息素	
	double Transition1(int i, int j);	//计算当前节点到下一节点转移的概率
	void UpdateLocalPathRule1(int i, int j);	//局部更新规则	
	void UpdateGlobalPathRule1(int* bestTour, int globalBestLength);	//全局更新规则
	~ACS1()
	{};  //析构函数
};


void ACS1::InitParameter1(double value) 	//初始化各路径上的信息素强度为value
{
	for (int i = 0; i < N; i++)
	{
		for (int j = 0; j < N; j++)
		{
			...
			if (i != j)
			{
				visible[i][j] = 1.0 / Mutual_Distance1[i][j];   //启发式信息强度
...
double ACS1::Transition1(int i, int j)//计算当前节点到下一节点转移的概率
{
	if (i != j)
	{
...
void ACS1::UpdateLocalPathRule1(int i, int j) //局部更新规则
{
...
void ACS1::UpdateGlobalPathRule1(int* bestTour, int globalBestLength)//全局更新规则
{
	for (int i = 0; i < N; i++)
	{
...
public:
	ACSAnt1(ACS1* acs, int start)  //构造函数
	{
		antColony = acs;
		startCity = start;
	}
	int* Search1();	//开始搜索	
	int Choose1();	//选择下一节点	
	void ant_numoveToNextCity1(int nextCity);	//移动到下一节点
	~ACSAnt1()
	{}; //析构函数
};


int* ACSAnt1::Search1()  //开始搜索
{
	...
	do
	{
		...
		toCity = Choose1();	     //选择下一个节点	
		if (toCity >= 0)
		{
			ant_numoveToNextCity1(toCity);    //移动到下一个节点
			antColony->UpdateLocalPathRule1(endCity, toCity);  //进行局部更新
...
int ACSAnt1::Choose1()  //选择下一节点，返回下一节点标号nextCity
{
	int nextCity = -1;
	...
	if (q <= q0)	//如果 q <= q0,按先验知识，否则则按概率转移
	{
		...
		for (int i = 0; i < N; i++)
		{
			...
			if (1 == allowed[i])
			{
				double prob = antColony->Transition1(cururentCity, i);  //计算当前节点转移到下一节点的概率
				if (prob  > probability)
				{
					nextCity = i;
					probability = prob;
	...
	else
	{
		...
		for (int j = 0; j < N; j++)
		{
			...
			{
				probability += antColony->Transition1(cururentCity, j) / sum; //往城市j转移的概率
...
void ACSAnt1::ant_numoveToNextCity1(int nextCity) //移动到下一节点
{
...
void Process_berlin52()
{
	...
	CalculateDistance1();	//计算表示两两城市之间的距离
	FILE *total_result;
	...
	for (int test_num = 0; test_num < 30; test_num++)
	{
		...
		double initInfo = 1 / (N * Cm1);	//各条路径上初始化的信息素强浓度
		acs->InitParameter1(initInfo);	  //初始化各路径信息素浓度
	...
	fprintf(total_result, "平均值:   \t%4.3fS\t%4.0f\n", sum_time / 30, total_length / 30);
	fprintf(total_result, "已公布最佳路径长度最好结果：7542\n");
