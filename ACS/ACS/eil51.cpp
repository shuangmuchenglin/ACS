...
using namespace::std;
const int N = 51; //城市数
int coordinate[N][2]; //坐标
int best_path[N];   //存储已公布的最优路径


#define MAX_ITER 2000  //最大迭代次数
#define ANT_NUM 10  //蚂蚁数量
...
double Mutual_Distance[N][N];	//表示两两城市之间的距离
double Cm;		//信息素局部更新时候使用的的常量，由贪心算法得到
...
void Print_bestpath()  //打印已公布的最优解路径及长度
{
	int i;
	cout << "已公布最优路径为：" << endl;
	for (i = 0; i < N - 1; i++)
	...
	cout << "最优解最小路径长度为：426" << endl;
}

int calculateDistance(int i, int j)//计算两个城市之间的距离(欧氏距离)
{
...
void CalculateDistance()   //用矩阵表示两两城市之间的距离
{
	for (int i = 0; i < N; i++)
	{
		for (int j = 0; j < N; j++)
...
public:
	ACS()
	{}  //构造函数
	void InitParameter(double value);	//初始化各路径信息素	
	double Transition(int i, int j);	//计算当前节点到下一节点转移的概率
	void UpdateLocalPathRule(int i, int j);	//局部更新规则	
	void UpdateGlobalPathRule(int* bestTour, int globalBestLength);	//全局更新规则
	~ACS()
	{};  //析构函数
};


void ACS::InitParameter(double value) 	//初始化各路径上的信息素强度为value
{
	for (int i = 0; i < N; i++)
	{
		for (int j = 0; j < N; j++)
		{
			...
			if (i != j)
			{
				visible[i][j] = 1.0 / Mutual_Distance[i][j];   //启发式信息强度
...
double ACS::Transition(int i, int j)//计算当前节点到下一节点转移的概率
{
	if (i != j)
	{
...
void ACS::UpdateLocalPathRule(int i, int j) //局部更新规则
{
...
void ACS::UpdateGlobalPathRule(int* bestTour, int globalBestLength)//全局更新规则
{
	for (int i = 0; i < N; i++)
	{
...
public:
	...
	int* Search();	//开始搜索	
	int Choose();	//选择下一节点	
	void ant_numoveToNextCity(int nextCity);	//移动到下一节点
	~ACSAnt()
	{}; //析构函数
};
...
int* ACSAnt::Search()  //开始搜索
{
	...
	do
	{
		...
		if (toCity >= 0)
		{
			ant_numoveToNextCity(toCity);    //移动到下一个节点
...
int ACSAnt::Choose()  //选择下一节点，返回下一节点标号nextCity
{
	int nextCity = -1;
...
void ACSAnt::ant_numoveToNextCity(int nextCity) //移动到下一节点
{
...
double CalAdjacentDistance(int node)//辅助函数，给一个节点由最近邻距离方法计算长度（计算Cm）
{
	double sum = 0.0;
	int visitedNode[N];
	for (int j = 0; j < N; j++)
	...
	do
	{
		...
		if (nextNode >= 0)
		{
			sum += Mutual_Distance[currentNode][nextNode];  //Mutual_Distance为两城市间的距离矩阵
...
void Process_eil51()
{
	...
	CalculateDistance();	//计算表示两两城市之间的距离
	FILE *total_result;
	...
	for (int test_num = 0; test_num < 30; test_num++)
	{
		...
		double initInfo = 1 / (N * Cm);	//各条路径上初始化的信息素强浓度
		acs->InitParameter(initInfo);	  //初始化各路径信息素浓度
	...
	fprintf(total_result, "平均值:   \t%4.3fS\t%3.0f\n", sum_time / 30, total_length / 30);
	fprintf(total_result, "已公布最佳路径长度最好结果：426\n");
